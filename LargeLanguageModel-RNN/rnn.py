...
class RNN(nn.Module):
    ...
    def step(self, input: Tensor, hidden_prev: Optional[Tensor] = None) -> Tensor:
        ...
        if hidden_prev is None:
            ...
            last_hidden_state = torch.zeros(input.size(0), self.hidden_dim)
            last_hidden_state = last_hidden_state.to(input.device)
        ...
        # print("next hidden shape:", next_hidden_state.shape)
        next_output_state = self.out(next_hidden_state)
        # print("next out shape:", next_output_state.shape)
...
class SelfAttention(nn.Module):
    ...
    def step(self, y_all: Tensor) -> Tensor:
        ...
        weights = torch.softmax(torch.matmul(query, keys.mT), dim=-1)
        # print("weights:", weights)

        # TODO: Compute weighted sum of values based on attention weights
        output_state = torch.sum(weights.mT*values, dim=1)
        # print("output state:", output_state.shape)
        # print("value state:", values.shape)
...
def train(lm, train_data, valid_data, loss_fn, optimizer, num_sequences, batch_size):
    ...
    for idx, sequence in enumerate(dataset):
        ...
        loss = loss_fn(token_logits.permute(0, 2, 1)[:, :, :-1], sequence[:, 1:])
...
def validate(lm, dataset, loss_fn):
    ...
    for i, sequence in enumerate(dataset):        
        if i < num_batches:
            ...
            loss = loss_fn(token_dists.permute(0, 2, 1)[:, :, :-1], sequence[:, 1:])
